...
# Color codes for tilstandsklasse
...
# ALS parameter name mapping to standard codes
ALS_PARAM_MAP = {
    'Arsen': 'As',
    'Kadmium': 'Cd',
    'Krom': 'Cr',
    ...
    'Sum alifater > C12-C35': 'Alifater_C12-C35',
    'TOC': 'TOC',
    'Tørrstoff': 'DryMatter',
}
...
def get_tilstandsklasse_from_color(color_rgb: str) -> int:
    ...
    if color_rgb and color_rgb in COLOR_TO_TILSTANDSKLASSE:
        return COLOR_TO_TILSTANDSKLASSE[color_rgb]
    return None
...
def extract_data(excel_path: Path) -> tuple[list, list]:
    ...
    for col, sample_name in sample_cols:
        ...
        for row in range(ROW_DATA_START, ws.max_row + 1):
            ...
            # Get parameter code
            param_code = ALS_PARAM_MAP.get(param_name, get_parameter_code(param_name))
...
def extract_kornfordeling() -> tuple[dict, list]:
    """
    Return hardcoded grain size distribution data for HTV bunnrensk (May 2025).
    ...
    # Hardcoded grain size distribution (Laben + >63mm Kumulativ %)
    grain_sizes = [
        (0, 0),
        (0.002, 0.22),
        (0.004, 0.61),
    ...
    results = []
    for size_mm, cumul_pct in grain_sizes:
...
def main():
    ...
    wide_df.to_excel(OUTPUT_DIR / 'p01_results_wide.xlsx', index=False)
